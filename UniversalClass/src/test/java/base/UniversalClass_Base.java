 ...
 * Date:08/01/2020
 * Description: Common base class which would be used in different class
 * */
...
public class UniversalClass_Base {
	...
	public void before_scenario()
	{
		...
		htmlreport = new ExtentHtmlReporter(prop.getProperty("reportslocation")+"\\signup.html");
		 ext = new ExtentReports();
		 ext.attachReporter(htmlreport);
		 ext.setSystemInfo("Host Name", "Mysystem");
		 ext.setSystemInfo("Environment", "Test Env");
		 ext.setSystemInfo("User Name", "Akanksha Yadav");
		   
		 htmlreport.config().setDocumentTitle("UniversalClass");
		 htmlreport.config().setReportName("UniversalClass Functional Testing");
	...
	{
		File f=((TakesScreenshot)driver).getScreenshotAs(OutputType.FILE);	//take screenshot of the webpage
		try
		{
